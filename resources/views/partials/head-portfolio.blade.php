@php
    $applicationName = data_get($portfolio ?? [], 'brand.name', config('portfolio.brand.name', config('app.name', 'ERICKsky')));
@endphp
...
{{-- Portfolio uses its own theme (see layouts/portfolio). Omit @fluxAppearance so Flux does not force system/light and remove .dark. --}}
<style>
    :root.dark {
        color-scheme: dark;
    }
